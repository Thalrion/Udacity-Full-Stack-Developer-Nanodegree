...
### NOTES BY STUDENT
1. Flashes after DB Insert:
Not requested, but I think its more intuitiv
...
Therefor, I also changed the templates/layouts/main.html file, 
so the type gets passed in as an argument to the render_template function
2. Form could not be validated due to a missing csrf-token.
I solved this issue by putting a "{{ form.csrf_token() }}"
under the respective <form> tag in forms/new_venue.html and forms/new_artist.html
3. I changed the way how strings are concationated (also from pre-existing codes) to `.format()`